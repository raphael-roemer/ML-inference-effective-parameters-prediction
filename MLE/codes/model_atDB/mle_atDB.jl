# wenn zu Beginn in DeadBand: kein Problem: entweder
#    1) wir bleiben im deadband, dann bleibt controll halt aus, oder
#    2) wir gehen raus, dann control an: ab posOfdbcrossing wird hoch gerampt

# wenn zu Beginn außerhalb des Deadbands:
# nehme eine Version, in der der control die ganze Zeit an ist.
...
function halfDetTimeseries_fpeRev(par) #par: [gamma1,gamma2,D,var_ww_0,var_aa_0,cov_wa_0]
    ...
    elseif posOfdbcrossing <0 # always outside db -->control on
...
for h in 7001:7400
            ...
            if abs(i_smoothfs3[1]) <= (0.01*2*pi)
                for i in 1:299
                    if abs(i_smoothfs3[i+1])>(0.01*2*pi)
                        ...
                        global turnOn=maximum([5,minimum([30,convert(Int64,abs(round((20/0.0045)*q)))])])
                        break
                    else
    ...
    println(h)
    writedlm("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_atDB/RES_model_atDB/"*String("$h.csv"),info[h,:])
end
...
plot(0:899,fs[((h-1)*3600+1):((h-1)*3600+900)])
plot!(0:899,timeseries_fpe(info[h,3:12])[:,1])
plot!(0:899,(timeseries_fpe(info[h,3:12])[:,1]+sqrt.(timeseries_fpe(info[h,3:12])[:,2])))
plot!(0:899,(timeseries_fpe(info[h,3:12])[:,1]-sqrt.(timeseries_fpe(info[h,3:12])[:,2])))
