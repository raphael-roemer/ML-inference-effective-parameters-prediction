...
# idee: mleres eröffnen
# durch alle dateien im ordner gehen und jeweils aus dem dateinamen den speicherort in
# mleres festlegen
dataf_t=readdir("./model_always/RES_model_always/")
dataf=dataf_t[2:length(dataf_t)-1]
mleres=Array{Float64,2}(undef,length(dataf),16)
i=1
for i in 1:length(dataf)
    mleres[parse(Int64,dataf[i][1:length(dataf[i])-4]),:]=readdlm("./model_always/RES_model_always/"*dataf[i])
end
mleres
Plots.plot(1:4500,mleres[1:4500,2])
#we see: at 1617 we start to jump
#mleres=mleres[1:200,:]

global PeakPost=readdlm("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/peakpos_atPeak.csv")[:,1]
PeakPos=Array{Int64,1}(undef,8760)
for i in 1:length(PeakPos)
    if isnan(PeakPost[i])==true
        PeakPost[i]=0
    end
    PeakPos[i]=convert(Int64,PeakPost[i])
end
PeakPos
...
goods2
mleres
length(goods2)/length(mleres[:,1])
length(goodfit)/length(mleres[:,1])
...
plotInt(hourp)
lkj
#png("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/plots/intsWc/1_"*string(hourp))
...
goods2Peakt=Array{Int64,1}(undef,length(goods2))
j=1
for i in goods2
    if PeakPos[i]!=0
        goods2Peakt[j]=i
...
goods2Peak=goods2Peakt[1:j-1]
PeakPos[goods2Peak]



# 1a) (q+r*t_nad)/gamma1 vs nadir   (remark: just intervals with control on - otherwise we would get infinities)
Plots.plot(abs.((mleres[goods2Peak,5].+(mleres[goods2Peak,6].*PeakPos[goods2Peak]))./mleres[goods2Peak,3]),
    mleres[goods2Peak,15],
    seriestype=:scatter,
...
corspearman(abs.((mleres[goods2Peak,5].+(mleres[goods2Peak,6].*PeakPos[goods2Peak]))./mleres[goods2Peak,3]),mleres[goods2Peak,15])
png("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/plots/validationNadir/1_NadScat")
@. prop(x,p)=p[1]*x
nadpropfit=curve_fit(prop,abs.((mleres[goods2,5].+(mleres[goods2,6].*mleres[goods2,2]))./mleres[goods2,3]),mleres[goods2,15],[1.0])
@. lin(x,p)=p[1]*p[2]
nadlinfit=curve_fit(prop,abs.((mleres[goods2,5].+(mleres[goods2,6].*mleres[goods2,2]))./mleres[goods2,3]),mleres[goods2,15],[1.0])
# now mark q
...
plot(abs.((mleres[nad_val,5].+(mleres[nad_val,6].*mleres[nad_val,2]))./mleres[nad_val,3]),
    mleres[nad_val,15],
...
nad_vallinfit=polyfit(abs.((mleres[nad_val,5].+(mleres[nad_val,6].*mleres[nad_val,2]))./mleres[nad_val,3]),mleres[nad_val,15],1)
plot!(0:1,nad_vallinfit(0:1),label=latexstring("\\textrm{lin.fit: }"*string(round(nad_vallinfit[1]; digits=2))*"x+"*string(round(nad_vallinfit[0]; digits=2))))
@. prop(x,p)=p[1]*x
...
png("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/plots/validationD/1_DScat")
...
plovald=density_plot((0.0,0.013),
    (0.0,0.005),
    (mleres[goods2,7].^2)./(2*mleres[goods2,3]),
    vari2[goods2],
    abs.(1000*mleres[goods2,3]),
    27,
    20)
Plots.plot(plovald[1],
    ...
    markeralpha=0.99)
plot!(0:0.0001:0.013,0:0.0001:0.013,label="\$\\textrm{expected}\$",color=:green)
png("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/plots/validationD/1_DScat_g1Col")
...
plot!(0:0.0001:0.013,0:0.0001:0.013,label="\$\\textrm{expected}\$",color=:green)
png("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/plots/validationD/1_DScat_DCol")
...
plot!(0:0.0001:0.013,0:0.0001:0.013,label="\$\\textrm{expected}\$")
png("/Users/raphaelbiertz/Documents/masterarbeit/coding/codes/model_always/plots/validationD/1_DScatMu")
